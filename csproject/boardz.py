    ...
    def game(self):
        ...
        while True:
            ...
            while True:
                ...
                mx, my = int(np.floor(point.getX())), int(np.floor(point.getY()))
                if not (0 <= mx < 8 and 0 <= my < 8):
                    print("Click within the board.")
                    continue
                if self.board[mx][my] != current_player:
            ...
            while True:
                point = self.win.getMouse()
                nx, ny = int(np.floor(point.getX())), int(np.floor(point.getY()))
                if not (0 <= nx < 8 and 0 <= ny < 8):
                    print("Click within the board.")
                    continue
                if (nx, ny) in movementable:
                    # Move the piece
                    ...
                    print("Invalid move. Please select a highlighted square.")
                    continue
